...
BashaCare is a Next.js App Router app for one building in Uttara. Visual source of truth is `DESIGN.md`. Product source of truth is `BRD.md`. Structure is `.design/basha-care/INFORMATION_ARCHITECTURE.md`.

## Blocks (repo notes)

Canonical vendored skills: `.codex/skills/<name>/SKILL.md` (see the imported block below). Cursor also has a copy at `.cursor/skills/`. If they differ, `.codex` wins.

CLI account `noor` (`noor.mohammad@selisegroup.com`) can reach project `D975c4874bd6b47b995cce54f926c09cc` (BashaCare, dev). Do not invent a substitute tenant. Pass `--account noor --project D975c4874bd6b47b995cce54f926c09cc` on project commands.

## UI

Follow `DESIGN.md`. No Inter, no purple gradients, no color-only status. Emergency is terracotta and full-bleed. Money is ৳.
